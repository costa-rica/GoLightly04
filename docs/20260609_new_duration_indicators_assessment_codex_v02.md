...
3. Recommended nullability:
   ...
   - Use `null` for pending, processing, failed, legacy, or manually recreated meditations that have not completed a new build yet.
...
6. Existing meditation handling:
   - Do not include a backfill script or historical-duration reconstruction in the recommended implementation scope.
   - Back up existing meditation scripts before the change is rolled out.
   - Manually recreate existing meditations after the backup, so recreated rows pass through the new worker path and receive freshly measured fields.
   - Treat this as both a process test and a clean start for the new duration metadata in the database.
...
### Risk 4: existing meditations need a manual recreation plan

Severity: moderate.

Existing complete meditations will show no guidance value until they are manually recreated. That may make the web table look inconsistent during the transition.

Mitigation:

- Back up scripts for existing meditations before rollout.
- Recreate the existing meditations manually after the new fields are deployed.
- Use the manual recreation process as a validation pass for the new creation pipeline and a fresh database start for duration metadata.
...
## Unknowns / Questions for Nick

### What should "guidance" mean?
...
_Placeholder for Nick's answer._

### What should the web table show?
...
### Should manually recreated meditations replace historical rows or create new rows?

The current recommendation assumes existing meditation scripts are backed up and then meditations are manually recreated after the new worker fields are deployed. Nick should confirm whether recreated meditations should replace existing library entries, coexist temporarily for review, or follow another operator workflow.

### Nick response

_Placeholder for Nick's answer._

### Should decimal pause durations be preserved more precisely than integer seconds?

The proposed database fields are integer seconds to match `duration_seconds`. If pause precision matters for reporting, the schema may need a decimal field type or a documented rounding rule.
