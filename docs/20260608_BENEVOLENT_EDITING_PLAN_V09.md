...
# Benevolent Meditation Editing — Architecture and Implementation Plan V09

## 1. Scope and Changes from V08

This revision supersedes V08 in one area identified by the Codex assessment of V08:

**V08's revised handler flow (§5 steps 10–11) omits the required success audit log.** Steps 10 and 11 read "Apply updates and call `.save()`" and "Serialize and return `200` with `AdminMeditation`", with no mention of emitting the structured `logger.info` entry that V01 defined and V02 preserved. Implemented literally, a successful edit would be saved and returned with no before/after audit trail — an auditability and security gap on the only admin-mutating path in this feature.

V09 expands steps 10–11 into four explicit steps in the canonical handler order: capture previous metadata before mutation; apply allowed updates; save; emit the structured success audit log; serialize and return. A specification block for the audit log payload is added, and the test contract is extended to require assertions that the logger is called with the before and after values.

All other V08 fixes remain valid and unchanged.
...
All files from V04 §4 apply. No new files are added by V09.

| File | Change vs V08 |
|------|--------------|
| `api/src/routes/admin.ts` | Handler steps 10–11 expanded to four explicit steps: capture previous; apply updates; save; emit audit log; return |
| `api/src/routes/admin.test.ts` (or equivalent test file) | Test contract extended: success case must assert logger called with correct before/after payload |

---

## 5. Validation and Handler Contract (revised, supersedes V08 §5)
...
10. **[restored from V02]** Capture `previous` metadata snapshot before mutation: `{ title: meditation.title, description: meditation.description ?? null, visibility: meditation.visibility }`.
...
After `.save()` completes (step 12), before serializing the response (step 14):

```ts
const previous = {
  title: meditationBeforeSave.title,
  description: meditationBeforeSave.description ?? null,
  visibility: meditationBeforeSave.visibility,
};

// ... apply updates, then await meditation.save() ...
...
`previous` is captured from the in-memory Sequelize instance fields before any mutation is applied (step 10). After `.save()`, the same instance fields hold the committed next values. `BENEVOLENT_USER_EMAIL` is the backend constant from `getOrCreateBenevolentUser.ts`; it is already imported in `admin.ts` for the owner guard. No user-supplied value is written to the log.
...
## 6. Test Contract (revised, supersedes V08 §6)

### 6.1 Body rejection test cases

Unchanged from V08 §6.1:
...
All notes from V04 §6 apply. The V09 change is additive: it restores logging that was specified in V01 and V02 but accidentally dropped in the V08 rewrite of the handler flow section. No guard behavior, error codes, response shapes, or DB semantics are changed.
...
**Risk from V09**: The `previous` snapshot must be taken from the in-memory instance fields before any field assignment. If the capture is placed after a partial field assignment (e.g., inside a loop that also mutates), `previous` will reflect a partially-mutated state and the log will be inaccurate. Mitigation: the capture is a single destructuring of `{ title, description, visibility }` from the Sequelize instance before the mutation block, as shown in §5.5.
