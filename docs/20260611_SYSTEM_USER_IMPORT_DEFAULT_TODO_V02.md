...
- [ ] Add import lookup/create response types for the dedicated import API.
...
- [ ] Add an authenticated owner-scoped import lookup endpoint, e.g. `GET /meditations/imports?sourceUserKey=...&sourceFile=...`.
...
- [ ] Return enough data for the importer to decide duplicate skip, overwrite, or create.

### TODO 7. Add dedicated import create/overwrite API contract

- [ ] Add a dedicated authenticated import endpoint, or equivalent import mode, that creates private script-mode meditations with validated provenance metadata.
- [ ] Preserve normal script parsing and worker notification behavior.
- [ ] Reject unsafe or unknown provenance shapes.
- [ ] Implement `overwrite` as delete/recreate:
...
- [ ] Add tests/response shapes for create, duplicate skip, and overwrite delete/recreate.
...
- [ ] Complete this before changing ordinary list filtering.
...
- [ ] Ensure default rows are not hidden from `GET /meditations/default` or from the frontend default surface.
...
- [ ] Load credentials from `/home/nick/agents_home/hermes/secrets/.env` without printing secrets.
...
- [ ] Implement `--dry-run`, duplicate skip, and `--overwrite` using the delete/recreate API contract from TODO 7.
