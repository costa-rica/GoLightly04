...
# GoLightly04 Data Backup & Restore — Plan V04

Source PRD: [[20260607_DATA_BACKUP_RESTORE_PRD]]
Supersedes: [[20260607_DATA_BACKUP_RESTORE_PLAN_V03]]
...
One correctness issue identified in the Codex V03 assessment is resolved here.
All V03 content (safeExtractZip helper, streaming extraction, entry-level
validation, defense-in-depth with safeRestoreResources) is carried forward
unchanged except for the import path fix below.

| Concern | V03 behavior | V04 correction |
|---|---|---|
| `safeExtractZip` imports logger from a non-existent path | `import { logger } from "./logger"` — no `api/src/lib/logger.ts` exists | Change to `import { logger } from "../config/logger"` — the API logger lives at `api/src/config/logger.ts` |
