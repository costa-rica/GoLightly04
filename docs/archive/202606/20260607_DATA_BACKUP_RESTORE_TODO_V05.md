...
# GoLightly04 Data Backup & Restore — TODO V05

Source plan: [[20260607_DATA_BACKUP_RESTORE_PLAN_V05]]
Source PRD: [[20260607_DATA_BACKUP_RESTORE_PRD]]
Supersedes: [[20260607_DATA_BACKUP_RESTORE_TODO_V04]]
...
| V04 defect | Phase | V05 correction |
|---|---|---|
| Phase 5d calls `safeRestoreResources` without any task creating it — leaves an undefined symbol and fails `npm run typecheck` | 5d | Add task **5c-r** to create `api/src/lib/safeRestoreResources.ts` and its unit tests before Phase 5d wires it into `database.ts` |

All V04 fixes are preserved: safeExtractZip streaming helper (5c), corrected
...
(5c), disk-upload multer (5a), restore confirmation modal (6b), and all test
tasks.
