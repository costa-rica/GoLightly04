...
# GoLightly04 Data Backup & Restore — Plan V05

Source PRD: [[20260607_DATA_BACKUP_RESTORE_PRD]]
Supersedes: [[20260607_DATA_BACKUP_RESTORE_PLAN_V04]]
...
One correctness issue identified in the Codex V04 assessment is resolved here.
All V04 content (safeExtractZip helper, logger import correction, streaming
extraction, entry-level validation) is carried forward unchanged except for
the safe resource restore section below.

| Concern | V04 behavior | V05 correction |
|---|---|---|
| `safeRestoreResources` implementation absent from plan | V04 says "See V02 plan for full implementation"; the helper does not exist in the repo — an implementer following V04 alone would either leave an undefined symbol or re-implement without the required safety rules | V05 promotes `safeRestoreResources` to a dedicated, independently testable file `api/src/lib/safeRestoreResources.ts` and provides its complete, implementable contract in this plan |
...
*(All V04 assumptions carry forward.)*
