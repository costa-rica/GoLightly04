...
- Commit message format per [AGENTS.md](../AGENTS.md): lowercase title ≤ 50 chars, body summarizing scope, `co-authored-by:` trailer; reference this file and the phase (e.g., `feat: phase 2 — db-models and shared-types (docs/20260609_DURATION_INDICATORS_TODO_V01.md)`).
...
- [ ] Write `scripts/backfill-segment-durations.ts` following the structure of `scripts/backfill-meditation-durations.ts`:
  - Supports `--apply`, `--force`, and `--limit` flags.
...
- [ ] Run `npm run backfill:segment-durations -- --help` and confirm it resolves without TypeScript or runtime errors.
- [ ] Run `npm run typecheck:scripts` and confirm it passes.

**Per-phase gate:**
- [ ] `jobs_queue` pre-check above was run and the result was acceptable
- [ ] `npm run backfill:segment-durations -- --help` resolves correctly
...
- [ ] After the worker completes, query the meditation again and confirm:
  ...
  - Values are plausible given the meditation's element list (e.g., a meditation with only a short text segment should have a non-zero `duration_seconds_talking` and zero or null for the others if no sound/pause elements are present).
