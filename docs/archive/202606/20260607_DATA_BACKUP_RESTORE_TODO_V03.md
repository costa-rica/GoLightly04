...
# GoLightly04 Data Backup & Restore — TODO V03

Source plan: [[20260607_DATA_BACKUP_RESTORE_PLAN_V03]]
Source PRD: [[20260607_DATA_BACKUP_RESTORE_PRD]]
Supersedes: [[20260607_DATA_BACKUP_RESTORE_TODO_V02]]
...
| V02 defect | Phase | V03 correction |
|---|---|---|
| `unzipper.Extract({ path: tempDir })` writes all archive entries before any safety check runs | 5c | Replace with `safeExtractZip` — a streaming helper that validates each entry name before writing; unsafe entries are rejected before any write occurs |
| No tests for crafted traversal/unsafe zip entries | 5e | New tests in Phase 5e prove that path-traversal, absolute, drive-prefixed, and non-allowlisted entries produce no writes outside the intended temp dir |

All V02 fixes are preserved: worker acceptance propagation (5b, 5c),
lstat backup walk (3c), manifest validation (5c), safeRestoreResources (5c),
disk-upload multer (5a), restore confirmation modal (6b), and all test tasks.
...
### 5c — Safe zip extraction helper (V03 — new)
...
- [ ] Create `api/src/lib/safeExtractZip.ts` with `safeExtractZip(zipPath,
      destDir): Promise<SafeExtractResult>` and helpers `isEntryNameSafe` and
      `isEntryAllowed`. Implement exactly as specified in the plan:
