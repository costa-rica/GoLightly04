...
# Dark Theme Color Change Plan — V03

## 0. Changes from V02

Supersedes [20260520_DARK_THEME_COLOR_CHANGE_PLAN_V02.md](20260520_DARK_THEME_COLOR_CHANGE_PLAN_V02.md). One issue from [20260520_DARK_THEME_COLOR_CHANGE_PLAN_V02_ASSESSMENT_CODEX.md](20260520_DARK_THEME_COLOR_CHANGE_PLAN_V02_ASSESSMENT_CODEX.md) drove this revision, plus one dead-weight instruction caught during verification:

1. **`.input-field` retrofit removed.** V02 told Phase 0 to update `.input-field` in [globals.css](web/src/styles/globals.css) so existing usages would pick up tokens automatically. The only `.input-field` consumers in the codebase are [ModalLogin.tsx](web/src/components/modals/ModalLogin.tsx) (lines 243, 267) and [ModalRegister.tsx](web/src/components/modals/ModalRegister.tsx) (lines 259, 284) — both explicitly out of scope per §7. Updating the helper would silently re-skin those inputs while their surrounding modal chrome (labels, dividers, helper text) stayed on the light-only palette, recreating the exact light-card/dark-input mismatch the plan is trying to remove, on out-of-scope surfaces. V03 leaves `.input-field` untouched and adds an explicit helper-class strategy in §3.
2. **`.card` retrofit removed.** Verification grep found zero consumers of `.card` in the codebase. The V02 instruction to update it was dead weight; dropped.

Locked-in decisions from V01 / V02 carry forward (semantic-token approach, modals one step lighter than cards, current accent saturation, admin scope = `admin/page.tsx` plus `AdminTable` and the five `TableAdmin*` components).
...
### Helper-class strategy (new in V03)
...
- [web/src/styles/globals.css](web/src/styles/globals.css) — declare the CSS variables for `:root` and `.dark` only. Do **not** modify the existing `.card` or `.input-field` helpers (see §3 Helper-class strategy).
...
- Add tokens to [tailwind.config.js](web/tailwind.config.js) per §3.
- Add the `:root` and `.dark` CSS-variable blocks to [globals.css](web/src/styles/globals.css). **Do not** edit `.card` or `.input-field`.
