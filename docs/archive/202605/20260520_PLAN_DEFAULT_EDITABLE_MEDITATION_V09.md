...
# Staged Default Meditation in Create Form (V09)

## Changes from V08

V09 addresses the concerns raised in `20260520_PLAN_DEFAULT_EDITABLE_MEDITATION_V08_ASSESSMENT_CLAUDE.md`.

1. The first-time staged create branch now explicitly sets `title: "Untitled staged meditation"` and `description: null`. `meditations.title` is non-null, so this is required for the first Generate path to work reliably.
2. The staged Generate transaction now states that the `FOR UPDATE` lock applies only if an existing staged row is found. If no row exists, the service takes the create branch and relies on the partial unique index plus retry for concurrent first Generate requests.
3. Metadata validation is no longer described as vague "existing rules." V09 introduces a shared `validateMeditationMetadata` helper used by both existing create routes and the new save-to-library service.

Everything else from V08 carries forward: shared `stage` response mapping, unified staged Generate service, exact template seed, benevolent owner, partial unique indexes, route ordering, library listing filter, ID-route access control, and admin guards.
...
This avoids copying the existing inline create-route validation into a third call site.
...
5. Use `stage: "staged"` from the unified staged Generate service create branch only if that branch delegates to this helper. Direct insert in the unified service is also acceptable if it preserves the same field normalization and JobQueue creation behavior.
...
3. `POST /meditations/staging/save-to-library`
   - Requires auth.
   - Uses `validateMeditationMetadata` for title, description, and visibility.
   - Validates the caller owns a staged row with `status='complete'`.
   - Atomically flips `stage` from `staged` to `library`.
   - Applies normalized title, description, and visibility.
...
4. Attempt to load the caller's existing staged row:
   ```ts
   Meditation.findOne({
     where: { userId, stage: "staged" },
     transaction,
...
14. `web/src/components/forms/CreateMeditationForm.tsx`.
15. `web/src/components/forms/ScriptMeditationEditor.tsx`.
16. `scripts/seedDefaultMeditation.ts`.
