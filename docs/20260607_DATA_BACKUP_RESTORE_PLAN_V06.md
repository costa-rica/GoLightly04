...
# GoLightly04 Data Backup & Restore — Plan V06

Source PRD: [[20260607_DATA_BACKUP_RESTORE_PRD]]
Supersedes: [[20260607_DATA_BACKUP_RESTORE_PLAN_V05]]
...
One correctness issue identified in the Codex V05 assessment is resolved here.
All V05 content (safeExtractZip helper, safeRestoreResources helper, logger
import correction, streaming extraction, entry-level validation) is carried
forward unchanged except for the uploadLarge multer export below.

| Concern | V05 behavior | V06 correction |
|---|---|---|
| `uploadLarge` disk-storage filename uses `file.originalname` | Multer joins the returned filename with `destination` before the route handler or `safeExtractZip` runs — a crafted multipart filename containing `..` segments or path separators can write the uploaded zip outside `os.tmpdir()`, reintroducing an extraction-before-validation filesystem-write risk at the upload layer | V06 removes `file.originalname` entirely; the filename is generated server-side using `crypto.randomUUID()` — the client-supplied filename is never used for any filesystem path construction |
...
| `api/src/lib/safeRestoreResources.ts` | Safe resource-copy helper (V05 — new, replaces inline V02 sketch) |
...
| `api/src/middleware/upload.ts` | Add `uploadLarge` (disk storage, 500 MB cap, server-generated filename) |
...
**V03 correction (carried forward):** Replace `unzipper.Extract({ path: tempDir })` with
...
2. `const zipPath = req.file.path` (path to temp file on disk — filename is
   server-generated; the client-supplied original filename is never used).
...
7. If manifest is valid and `package_type === "db_and_resources"`:
   call `await safeRestoreResources(tempDir, env.PATH_PROJECT_RESOURCES)`
   (V05 helper — `api/src/lib/safeRestoreResources.ts`; provides defense-in-depth
   for the resource-copy step).
...
**Defense-in-depth:** `safeRestoreResources` (V05) still runs after extraction
...
## New Helper: `safeRestoreResources` (V05 — `api/src/lib/safeRestoreResources.ts`)

**Why a separate file (V05 change from V02):** Earlier plans embedded
`safeRestoreResources` as an inline function inside `database.ts` or referred
implementers back to V02. That made the helper impossible to unit-test
independently and caused a missing-symbol typecheck failure when following V04
alone. V05 promotes it to `api/src/lib/safeRestoreResources.ts` so it can be
imported by `database.ts` and exercised by a dedicated test file.
...
- `tempDir` — the temp directory that `safeExtractZip` wrote to.
- `resourcesRoot` — `PATH_PROJECT_RESOURCES` from the API env (the live resource tree).
- Returns the number of regular files successfully copied into `resourcesRoot`.
...
export async function safeRestoreResources(
  ...
  // If the resources directory was not extracted (DB-only archive or missing),
  ...
  async function walk(dir: string): Promise<void> {
    ...
    for (const entry of entries) {
      ...
      // Verify the resolved source stays under srcRoot.
      const resolvedSrc = path.resolve(fullSrc);
      if (
        !resolvedSrc.startsWith(srcRoot + path.sep) &&
        resolvedSrc !== srcRoot
      ) {
        logger.warn(
      ...
      // Compute and verify the destination path stays under destRoot.
      const relPath = path.relative(srcRoot, resolvedSrc);
      const destPath = path.resolve(destRoot, relPath);
      if (
        !destPath.startsWith(destRoot + path.sep) &&
        destPath !== destRoot
      ) {
        logger.warn(
      ...
      // Reject entries whose top-level path segment is a backup directory.
      const topLevelSegment = relPath.split(path.sep)[0];
      if (EXCLUDED_RESTORE_DIRS.includes(topLevelSegment)) {
        logger.warn(
          `safeRestoreResources: skipping excluded directory entry ${relPath}`,
        );
        continue;
      }

      // Create parent directories and copy, overwriting any existing file.
      await fsPromises.mkdir(path.dirname(destPath), { recursive: true });
...
**Safety contract summary:**
- Source root is `path.resolve(tempDir, "resources")`.
- Destination root is `path.resolve(resourcesRoot)`.
- If `tempDir/resources` does not exist or is not a directory, returns `0` immediately.
- Recursively walks source with `fsPromises.lstat` on every entry — never `stat`,
  so symlinks are never silently followed.
- Skips symlinks with a `logger.warn`; does not copy them or recurse into them.
- Skips non-regular, non-directory entries (devices, FIFOs, sockets) with a
  `logger.warn`.
- For each regular file: resolves its absolute source path and verifies it starts
  with `srcRoot + path.sep` before computing the destination.
- Computes the relative path from `srcRoot`, resolves the absolute destination
  path under `destRoot`, and verifies it starts with `destRoot + path.sep` before
  writing.
- Rejects entries whose top-level path segment is `"backups_db"` or
  `"backups_db_and_data"` with a `logger.warn`.
- Creates destination parent directories as needed (`mkdir recursive`).
- Overwrites existing files (`copyFile` replaces by default).
- Returns the total count of files successfully copied.

**Import in `database.ts`:**
...
*(V06 — `file.originalname` removed; server-generated filename only.)*

The client-supplied filename is **never used** for any filesystem path
construction. Multer disk storage joins the returned filename with the configured
`destination` before the route handler or `safeExtractZip` runs, so including
any client-controlled value here would allow a crafted multipart `filename`
field (e.g. `../../evil.zip`) to write the uploaded zip outside `os.tmpdir()`.
The fix is to generate the basename entirely on the server.
...
**Security note:** The `_file` parameter (the multer `File` object carrying
`originalname`) is intentionally unused. No part of the client request influences
the path written to disk.

**Traversal test:** A crafted multipart upload with `filename` set to
`../../evil.zip` must be verified to produce `req.file.path` that remains under
`os.tmpdir()` and must not create a file at the traversal target. See Phase 5f
traversal tests in the TODO document for the required test or checklist item.
...
| `safeRestoreResources` implementation | ~60 lines in `api/src/lib/safeRestoreResources.ts`. Uses `fsPromises.lstat`, `path.resolve`, and `startsWith` for containment — well-established pattern. Independently testable via its own test file. |
| `uploadLarge` filename safety | `crypto` is a Node.js built-in; no new dependency. The `_file` parameter (carrying `originalname`) is unused. `crypto.randomUUID()` produces a sufficiently unique, filesystem-safe basename. |
...
| Upload-layer path traversal | Resolved in V06 by removing `file.originalname` from the disk filename callback. Server-generated filename means no client-controlled path segment reaches the filesystem before the route handler runs. |
| Testing crafted multipart filename | Node.js `multer` applies the filename callback and joins with destination before calling `next`. A unit test using `supertest` can upload a multipart form with `filename: "../../evil.zip"` and assert `req.file.path` is under `os.tmpdir()` and the traversal target was not created. |
...
| Testing `safeRestoreResources` | A temp directory with controlled file layout (symlinks, regular files, banned directories) is sufficient — no special infrastructure needed. |
| Legacy DB-only restore | `safeExtractZip` still writes root-level CSVs. `hasManifest: false` triggers DB-only path; `safeRestoreResources` is not called. No behavior change. |
| Disk multer for restore | Filename is now server-generated (V06 fix). |
...
*(All V05 assumptions carry forward.)*
...
9. `crypto` is a Node.js built-in available in all supported runtime versions;
   no `package.json` change is required for `import crypto from "crypto"`.
