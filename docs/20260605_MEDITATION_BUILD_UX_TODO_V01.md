...
- [ ] Add `scripts/backfill-sound-file-durations.ts` mirroring the existing meditation-duration backfill script:
  ...
  - [ ] Resolve each file path using the existing prerecorded-audio path helper rather than hard-coded paths.
...
## Phase 5 — Form-mode build animation
...
## Phase 6 — Fixed build-block palette and wider app containers

- [ ] Update `web/tailwind.config.js`:
  - [ ] Add `colors.buildBlock.text`, `colors.buildBlock.pause`, and `colors.buildBlock.sound` using the PRD’s indigo/amber/teal palette or close accessible values.
