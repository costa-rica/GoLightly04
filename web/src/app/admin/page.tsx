...
export default function AdminPage() {
  ...
  const handleMeditationDeleteConfirm = async () => {
    ...
    try {
      ...
      setToast({
        message: "Meditation and all related files deleted.",
        variant: "success",
      });
  ...
  const handleRequeueMeditation = async (record: QueueRecord) => {
    try {
      await requeueMeditation(record.meditationId);
      setToast({ message: "Meditation requeued.", variant: "success" });
      await fetchQueuerRecords();
      await fetchMeditations();
    } catch (err: any) {
      const message =
        err?.response?.data?.error?.message || "Unable to requeue meditation.";
      setToast({ message, variant: "error" });
    }
  };
      ...
      <main className="min-h-screen bg-canvas">
        <div className="mx-auto flex w-full max-w-app flex-col gap-10 px-4 py-10 md:px-8 md:py-16">
          <header className="rounded-3xl border border-subtle bg-raised p-8 shadow-sm backdrop-blur">
            ...
            <h1 className="mt-3 text-3xl font-display font-semibold text-ink md:text-4xl">
              Manage Go Lightly
            </h1>
            <p className="mt-3 max-w-2xl text-base text-ink-muted md:text-lg">
              Review user accounts, meditation content, sound files, and jobs
              queue records.
            </p>
          </header>

          <section className="space-y-4">
            <div className="flex w-full items-center justify-between rounded-2xl border border-subtle bg-raised px-4 py-3 text-left shadow-sm">
              <div>
                <h2 className="text-xl font-display font-semibold text-ink">
                  Users
                </h2>
                <p className="text-sm text-ink-muted">Manage registered users</p>
              </div>
              <button
                type="button"
            ...
            {isExpanded && (
              <div className="rounded-3xl border border-subtle bg-raised p-4 shadow-sm md:p-6">
                {loading && (
                  <div className="space-y-3">
                    {Array.from({ length: 4 }).map((_, index) => (
                      <div
                        key={`admin-users-skeleton-${index}`}
                        className="flex items-center justify-between rounded-2xl border border-subtle bg-inset px-4 py-3 animate-pulse"
                      >
                        <div className="h-4 w-1/3 rounded-full bg-strong/60" />
                        <div className="h-4 w-20 rounded-full bg-strong/60" />
                      </div>
                    ))}
                  </div>
                )}
          ...
          <section className="space-y-4">
            <div className="flex w-full items-center justify-between rounded-2xl border border-subtle bg-raised px-4 py-3 text-left shadow-sm">
              <div>
                ...
                <p className="text-sm text-ink-muted">
                  Jobs Queue mirrors the database table, one row per meditation
                  element.
                </p>
              </div>
              <button
              ...
              <div className="rounded-3xl border border-subtle bg-raised p-4 shadow-sm md:p-6">
                ...
                {!queueLoading && !queueError && (
                  <TableAdminQueuer
                    ...
                    onRequeue={handleRequeueMeditation}
                  />
                )}
              </div>
            )}
              ...
              <div className="rounded-3xl border border-subtle bg-raised p-4 shadow-sm md:p-6">
                  ...
                  <div className="space-y-3">
                    {Array.from({ length: 4 }).map((_, index) => (
                      <div
                        ...
                        className="flex items-center justify-between rounded-2xl border border-subtle bg-inset px-4 py-3 animate-pulse"
                      >
                        <div className="h-4 w-1/2 rounded-full bg-strong/60" />
                        <div className="h-4 w-24 rounded-full bg-strong/60" />
                      </div>
                    ))}
                  </div>
                )}
                ...
                {!databaseLoading && !databaseError && (
                  <div className="space-y-6">
                    ...
                    <div className="rounded-2xl border border-subtle bg-inset/60 p-4">
                      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                        ...
                        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                          <label className="rounded-full border border-subtle bg-raised px-4 py-2 text-xs font-semibold text-ink transition hover:border-primary-200">
                            Choose File
                            <input
                              type="file"
                              accept=".zip"
